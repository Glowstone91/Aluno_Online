...
<table class="table table-striped">
  ...
  <tbody>
	...
	data-bs-target="#editarmateriaModal" data-bs-id="<?php echo $materias['id']?>"
	data-bs-materias="<?php echo $materias['materias']?>"
	><i class="bi bi-pencil-square"></i></button>
	
	
	<button type="button" class="btn btn-outline-dark" data-bs-toggle="modal"
	data-bs-target="#excluirmateriaModal" data-bs-id="<?php echo $materias['id']?>"
	data-bs-materias="<?php echo $materias['materias']?>"><i class="bi bi-trash3-fill"></i></button>
...
<button data-bs-toggle="modal"
		  data-bs-target="#cadastro_materia" class="cadastro">Cadastrar</button>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Editar Matéria</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
	  <form method="POST" action="editar_materia.php">
		...
		<div class="mb-3">
         <label for="recipient-name" class="col-form-label">Matéria:</label>
            <input type="text" class="form-control" id="input_materia"  name="materia" placeholder="Matéria" required>
          </div>
      </div>
		<div class="modal-footer">
...
<!-- Excluir materia modal -->
<div class="modal fade" id="excluirmateriaModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
	  ...
	  <form method="POST" action="excluir_materia.php">
		<div class="modal-body">
		 <input type="hidden" id="input_id_excluir" name="id" required>
		 Deseja excluir essa matéria?	
		</div>
		<div class="modal-footer">
         <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
...
<div class="modal fade" id="cadastro_materia" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Cadastro de matéria</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
	  <form method="POST" action="cadastro_materia.php">
		<div class="modal-body">
          <div class="col-12">
            <label for="inputmateria" class="col-form-label">Matéria:</label>
            <input type="text" class="form-control" id="inputmateria"  name="materia" placeholder="Nome da matéria" required>
          </div>	   		  
		</div> 
		<div class="modal-footer">
         <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
...
var editarmateriaModal = document.getElementById('editarmateriaModal')
console.log("MODAL: ", editarmateriaModal)
editarmateriaModal.addEventListener('show.bs.modal' , function (event) {
  ...
  var materia = button.getAttribute('data-bs-materias');
	console.log("MATERIA: ", materia);
	document.getElementById("input_materia").value = materia;

})
</script>

<script type="text/javascript">
var excluirmateriaModal = document.getElementById('excluirmateriaModal')
console.log("MODAL: ", excluirmateriaModal)
excluirmateriaModal.addEventListener('show.bs.modal' , function (event) {
	console.log("EVENTO: ", event);
  ...
  var materia = button.getAttribute('data-bs-materias');
	...
	modalTitle.textcontent = 'Excluir' + materia
...
<?php
if(isset($_GET['update_materia'])){
	//var_dump($_GET);
	
	if($_GET['update_materia'] === 'true') {?>
	<script>
